...
AES-GCMやいくつかの暗号技術で利用される標数2の拡大体の解説や実装方法の紹介をします。今回はまず概念の説明です。

## 標数2の素体の演算
素体とは $p$ を素数としたとき0以上 $p-1$ 以下の整数の集合 $𝔽_p=\{0,1,\dots,p-1\}$ のことです。体というのは四則演算ができる集合です。加算、減算、乗算は普通に計算したと $p$ で割った余りを結果とします。割り算（逆数）は乗算と整合性が取れるように定めます。
$p$ が256ビット程度の大きな素数のときの話は[有限体の実装1（加算）](https://zenn.dev/herumi/articles/𝔽inite-𝔽ield-01-add)をご覧ください。
標数2の素体というのは $p=2$ のときの体、すなわち0と1しかない体 $𝔽_2=\{0,1\}$ です。要素が2個しかないので演算も簡単です。
...
これはビット演算の排他的論理和 ⊕ と同じです。すなわち $𝔽_2$ の加算は
```c
typedef bool F2;
...
この両辺から $x$ を引くと
$$x = -x.$$
これは $𝔽_2$ の世界でマイナスはプラスに変えても同じなので引き算は足し算と同じになります。
...
1×1 = 1以外は全て0になります。これはビット演算の論理積 & と同じです。こちらは
```c
...
### 除算
0以外の要素 $x$ の逆元 $r$ は $rx=1$ となる $r$ として定義し、$1/x=r$ と書きます。素体ではどの $x (\neq 0)$ に対しても逆元が存在することが知られています。
...
## 複素数
次に拡大体の話をします。が、$𝔽_2$ の拡大体の話の前に複素数の話をします。回り道をすっとばして[$𝔽_2$の8次拡大体](#$𝔽_2$の8次拡大体)にとんでも構いません。

複素数ℂとは実数ℝに虚数単位 $i$ を追加したものでした。$i$ は2乗して-1になる数です。ℂの要素は実数 $a$, $b$ を使って $z=a+bi$ と書きました。
$$ℂ=\{a+bi|a,b\in ℝ\}.$$
...
実数体に2次式 $X^2+1$ を使って作った体なので2次拡大体といいます。複素数体は実数の2次拡大体なのです。

## $𝔽_2$ の2次拡大体
前述と同様の考えを使って $𝔽_2$ を構成します。$X^2+1$ を使いたいところですが、残念ながらうまくいきません。
なぜなら $X^2+1$ は $𝔽_2[X]$ の中で因数分解できてしまうからです。
$$0=X^2+1=X^2+2X+1=(X+1)^2.$$
※ $𝔽_2$ の中では2倍したものは0と同じなことに注意。
これは $f=X+1$ という0でない要素同士を掛けると0になるということを意味します。
...
加減算の定義はそのままで、乗算は
$$zw = (a+bX)(c+dX)=bdX^2+(ad+bc)X+ac=bd(X+1)+(ad+bc)X+ac=(ac+bd)+(ad+b+bc)X.$$

※ $𝔽_2$ では $X^2→X+1$ という置き換えが成り立つことに注意する。

このようにして作った2次拡大体を $𝔽_{2^2}$ と書きます。$GF(2^2)$ と書くこともあります。GFはGalois Fieldの略です。
