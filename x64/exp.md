...
上記の`vrndscaleps`で求めたnの2のべき乗を掛けます。
これでexp(x)の計算が終了です。

## 2巾とvscalefps
AVX2までは整数nに対してfloat(2^n)は次のように求めていました。

floatは符号ビットs、指数部e、仮数部fからなります。
それぞれsが1ビット、eが8ビット、fが23ビットで合計32ビットです。
...
したがってビットパターンが`[s:e:f]`で表されるfloatは`(-1)^s × 2^(e-127) × (1 + f/2^23)`という値を表します。
よってfloat(2^n)を求めるには`vrndscaleps`の結果を整数型にキャストしてnを求めた上で、

```
((n + 127) << 23)
```
という32ビット整数のデータを生成し、それをfloat型と見なして乗算する必要がありました。
`vscalefps`はそれを1命令で実行できます。
127という定数値をSIMDレジスタに代入する必要がなくなり、float⇔intの型変換も無くなるので便利で高速です。
