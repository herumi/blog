...
で定まる素数 $p$ の有限体 $𝔽_p$ 上の $y^2=x^3+4$で定まる楕円曲線（$a=0$, $b=4$）です。
$E(𝔽_p)$ の中から素数位数 $r$ の生成元を選んでそれを使います。以下の楕円曲線の点は全て $E[r]=\Set{P\in E(𝔽_p)|rP = 0}$ の元とします。ペアリングの行き先が $𝔽_p$ の12次拡大体となります。
...
```
という結果になりました。$P$ を $L$ 倍した $Q$ の $y$ 座標 `Q.y` は `P.y` に等しく $Q$ の $x$ 座標 `Q.x` は `P.x` の $w$ 倍されていることを確認できました。
他の $P$ でも同様の結果になります。
